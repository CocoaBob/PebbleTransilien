...
static void accel_tap_service_tap_handler(AccelAxisType axis, int32_t direction) {
...
void accel_tap_service_init(AccelTapServiceHandler handler, void *context) {
    ...
    accel_tap_service_subscribe((AccelTapHandler)accel_tap_service_tap_handler);
}
